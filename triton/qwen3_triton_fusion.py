...
class GroupedQueryAttention(nn.Module):
    def __init__(
        ...
        self.qkv_dim = self.d_out + 2 * (num_kv_groups * head_dim)
        self.W_qkv = nn.Parameter(torch.rand(self.qkv_dim, d_in) / d_in**0.5)
        self.qkv_split_size = [self.d_out, num_kv_groups * head_dim, num_kv_groups * head_dim]
    ...
    def forward(self, x, mask, cos, sin, start_pos=0, cache=None):
        ...
        qkv = triton_matmul(x_f, self.W_qkv.T)
        q, k, v = torch.split(qkv, self.qkv_split_size, dim = -1)
        
        queries = q.reshape(b, num_tokens, self.num_heads, self.head_dim).transpose(1,2)
        keys_new = k.reshape(b, num_tokens, self.num_kv_groups, self.head_dim).transpose(1,2)
        values_new = v.reshape(b, num_tokens, self.num_kv_groups, self.head_dim).transpose(1,2)        
...
class Qwen3Model(nn.Module):
    ...
    def forward(self, in_idx, cache=None):
        ...
        if cache is not None:
            ...
            if num_tokens == 1:
                # Decode step: no quadratic mask needed
                mask = None
            else:
                # Chunked prefill into non-empty cache (rare), or first-call chunking
                # Build only the within-block causal part and prepend zeros for the past.
                ...
                if pos_start == 0:
                    ...
                    past = torch.zeros(num_tokens, pos_start, device=x.device, dtype=torch.bool)
                    mask = torch.cat([past, causal_block], dim=1)
        else:
            # Full prefill (no cache yet): standard causal mask for the block
            pos_start = 0
            mask = torch.triu(
                torch.ones(num_tokens, num_tokens, device=x.device, dtype=torch.bool),
                diagonal=1
...
def load_weights_into_qwen(model, param_config, params):
    ...
    for l in range(param_config["n_layers"]):
        ...
        q_weight = params[f"model.layers.{l}.self_attn.q_proj.weight"]
        k_weight = params[f"model.layers.{l}.self_attn.k_proj.weight"]
        v_weight = params[f"model.layers.{l}.self_attn.v_proj.weight"]
        ...
        att.W_qkv = assign(
            att.W_qkv,
            qkv_weight,
            f"model.layers.{l}.self_attn.qkv_proj.weight fused"
        )
        # Q, K, V projections
        # att.W_query = assign(
        #     att.W_query,
