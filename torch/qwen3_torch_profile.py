...
def main():
    ...
    # Warm-up (important for stable GPU kernels)
    with torch.no_grad():
        _ = model(input_token_ids_tensor)

    # Profile ~5 inference steps
    with profile(
        activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        record_shapes=True,
        profile_memory=True,
        with_stack=True,      # Required for Python source line correlation
    ) as prof:
        with torch.no_grad():
            for _ in range(5):
                with record_function("generate_step"):
                    out = model(input_token_ids_tensor)[:, -1]  # forward pass
                # simulate adding next token
                input_token_ids_tensor = torch.cat([input_token_ids_tensor, out.argmax(-1, keepdim=True)], dim=1)

    prof.export_chrome_trace("qwen_inference_trace.json")
    print("\nSaved trace to qwen_inference_trace.json") 
