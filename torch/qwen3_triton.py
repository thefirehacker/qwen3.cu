...
class GroupedQueryAttention(nn.Module):
    def __init__(self, d_in, num_heads, num_kv_groups, head_dim=None, qk_norm=False, dtype=None, start_pos=0, cache=None):
        super().__init__()
        ...
        self.W_key = nn.Parameter(torch.rand(num_kv_groups * head_dim, d_in) / d_in**0.5)
        self.W_value = nn.Parameter(torch.rand(num_kv_groups * head_dim, d_in) / d_in**0.5)
    ...
    def forward(self, x, mask, cos, sin, start_pos=0, cache=None):
        ...
        queries = triton_matmul(x_f, self.W_query.T).reshape(b, num_tokens, self.num_heads, self.head_dim).transpose(1, 2)   
        keys_new = triton_matmul(x_f, self.W_key.T).reshape(b, num_tokens, self.num_kv_groups, self.head_dim).transpose(1, 2)
        values_new = triton_matmul(x_f, self.W_value.T).reshape(b, num_tokens, self.num_kv_groups, self.head_dim).transpose(1, 2) 
